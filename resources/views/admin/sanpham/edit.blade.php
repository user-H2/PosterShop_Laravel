...
<div class="card shadow mb-4">
    <div class="card-header py-3 bg-dark">
        <h5 class="m-0 font-weight-bold text-gray-200">Sửa sản phẩm</h5>
    </div>
    <div class="card-body">
        <form action="{{route('admin.sp.update',$sp)}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="mb-3 row">
            ...
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    ...
                    <img src="/assets/img/{{$sp->image}}" style="width:100px" alt="">
                </div>
            </div>
            <div class="mb-3 row">
            ...
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    <textarea type="text" class="form-control" name="mota_sp" id="mota">{{$sp->mota_sp}}</textarea>
                </div>
            </div>
            <div class="mb-3 row">
                ...
                <div class="col-md-4">
                    <select name="loaisp" id="idloai" class="form-control">
                        @foreach($category as $item)
                            @if($item->id == $sp->id_loai_sp)
                                <option selected value="{{$item->id}}">{{$item->tenloai}}</option>
                            @endif
                ...
                <div class="col-md-4">
                    <select name="ncc" id="idncc" class="form-control">
                        @foreach($nhacungcap as $item)
                            @if($item->id == $sp->id_ncc)
                                <option selected value="{{$item->id}}">{{$item->ten_ncc}}</option>
                            @endif
